---
name: paseo-ship-slice
description: Ship one vertical slice — a GitHub child issue — from ticket to merge-ready PR inside a Paseo worktree workspace. Use when the user asks to implement or ship a slice, mentions /paseo-ship-slice, or a worker is prompted with `/paseo-ship-slice <issue> orchestrated` by /paseo-orchestrate-slice.
...
One slice = one child issue = one Paseo worktree workspace = one branch = one PR.
...
The orchestrator owns scheduling, parallelism, workspace lifecycle, parent state, merge serialization and authorization, and cleanup.

### Standalone mode

```text
/paseo-ship-slice <issue>
```
...
If this issue itself has implementation sub-issues, it is a tracker, not a slice — stop and direct the caller to `/paseo-orchestrate-slice`.
...
## 2. Verify the workspace and branch
...
The branch and Paseo worktree workspace already exist; this agent runs with the worktree as its `cwd`. Inspect:
...
Require: the checked-out branch is the assigned slice branch (never `$BASE`), the current worktree's entry in `git worktree list` points at `pwd` (this is a linked worktree, not the main checkout), and no unexplained pre-existing application-code changes exist.
...
Never create a replacement workspace or run another bootstrap yourself.
...
Then wait for the orchestrator, which owns the merge, workspace archiving, child/parent closing, and any next slice.
